            ...
            .with_context(|| format!("read completion directory {}", directory.display()))?
        {
            ...
            match load_entry(&path) {
                Ok((id, response, expires_at, in_flight)) if expires_at > now => {
                    // A claim that survived a process restart has an indeterminate
                    // outcome. Convert it into a durable tombstone rather than risk
                    // replaying a POST that may already have reached the upstream.
